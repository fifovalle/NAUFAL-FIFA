  ...
  return (
    <div className="flex flex-col">
      ...
      {journey.map((item, index) => {
        ...
        const maxVisible = 2;
        const hasExtra = item.gallery && item.gallery.length > maxVisible;

        return (
          <div key={index} className="flex items-start gap-12 w-full mb-8">
            {/* ============== BULLET POINT ================ */}
            ...
            <div className="max-w-125">
              ...
              {item.gallery && item.gallery.length > 0 && (
                <div className="grid grid-cols-2 gap-3 mt-4 px-2 xl:px-0">
                  {item.gallery.slice(0, maxVisible).map((img, i) => (
                    <div
                      key={i}
                      className="relative w-full h-32 rounded-lg overflow-hidden cursor-pointer"
                      onClick={() => {
                        if (i === maxVisible - 1 && hasExtra) {
                          setModalImageAll(item.gallery!);
                        } else {
                          setModalImage(img);
                        }
                      }}
                    >
                      <Image
                        priority
                        src={img}
                        alt={`${item.role} photo ${i + 1}`}
                        fill
                        className="object-cover"
                      />
                      {i === maxVisible - 1 && hasExtra && (
                        <div className="absolute inset-0 bg-black/60 flex items-center justify-center text-white text-xl font-bold">
                          +{item.gallery.length - maxVisible}
                        </div>
                      )}
                    </div>
                  ))}
